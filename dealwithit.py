...
def detectAndDisplay(frame, face_classifier, eye_classifier, glasses, scaleFactor=None, minNeighbors=None):
    ...
    for (x, y, w, h) in faces_rects:
        ...
        if len(eyes_rects) == 2:
            ...
            # print(eyes_rects)
            upper_left = np.array([eyes_rects[0][0]-100, eyes_rects[0][1]-50], dtype=np.float32)
            bottom_left = np.array([eyes_rects[0][0]-100, eyes_rects[0][1] + eyes_rects[0][3]+50], dtype=np.float32)
            bottom_right = np.array([eyes_rects[1][0]+100 + eyes_rects[1][2], eyes_rects[1][1] + eyes_rects[1][3]+50], dtype=np.float32)
            upper_right = np.array([eyes_rects[1][0]+100 + eyes_rects[1][2], eyes_rects[1][1]-50], dtype=np.float32)
            ...
            mask = cv2.warpPerspective(mask, M, (face.shape[1], face.shape[0]), cv2.INTER_LINEAR)
...
glasses = cv2.imread("dealwithit.png", flags=IMREAD_UNCHANGED)
print(glasses.shape)
plt.imshow(glasses)
plt.show()
